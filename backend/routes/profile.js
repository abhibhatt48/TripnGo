...
router.post('/', async function (req, res, next) {
  ...
  try {
    ...
    const user =await database.collection('UserProfile').find({ email }).toArray();
    console.log(user);
...
router.put('/:id', async function (req, res, next) {
    ...
    const user = await database.collection('UserProfile').findOne({ _id: objectId });
      ...
      await database.collection('UserProfile').updateOne({ _id: objectId}, { $set: user });
